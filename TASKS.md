...
- [~] Add explicit matrix/test-port coverage for `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/events).
  Baseline added in `crates/json-joy-core/src/model_api.rs` + `crates/json-joy-core/tests/upstream_port_model_api_matrix.rs` and `crates/json-joy-core/tests/upstream_port_model_api_events_matrix.rs` (`from_patches`, `apply_batch`, `find/read/select`, pointer-path variants with `-` append + escaped token handling, root `diff`, `merge`, core mutators, tolerant `add/replace/remove/op`, path-bound `NodeHandle` proxy-style mutations, typed wrappers `as_obj/as_arr/as_str/as_val/as_bin/as_vec/as_con`, per-patch and batch change fanout with subscribe/unsubscribe and local-vs-remote origin tagging). Remaining gap is generated proxy tree semantics and extension-specific API typing.
