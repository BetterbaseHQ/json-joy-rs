...
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_msgpack_util_matrix.rs` covering `msgpack/util.ts` helper parity (`encode`, `encodeFull`, `decode`, and exported `MsgPack` alias shape) with fast/full encoder roundtrip behavior.
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_msgpack_shallow_read_matrix.rs` covering `msgpack/shallow-read.ts` generator parity by asserting offset-equivalence against direct decoder path traversal across nested object/array paths and upstream error surfaces (`KEY_NOT_FOUND`, `INDEX_OUT_OF_BOUNDS`).
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_surface_types_matrix.rs` covering lightweight surface-family parity for `bencode/types.ts`, `cbor/types.ts`, `cbor/shared.ts`, `json/types.ts`, `json-binary/types.ts`, and `ws/errors.ts` (type alias visibility, shared CBOR helper roundtrip, and WebSocket encoding/decoding error class exports).
...
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_ion_matrix.rs` covering Ion binary encoder/decoder behavior from upstream `ion` suites, including scalar/container roundtrips, symbol-table annotation flows for object fields, strict error paths (invalid BVM, negative-zero nint, unknown symbol IDs), `IonDecoderBase` parity edges (NOP `null` semantics, invalid bool lengths, unknown type nibble errors, annotation-short errors, and container length mismatch errors), `IonDecoder.read()` streaming behavior, and a broad upstream-style encoder boundary inventory (integer/floating/string/binary/container matrices from `IonEncoder.spec.ts`, including 8-byte integer wire-shape boundaries and `i64::MIN` nint handling).
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_ion_import_matrix.rs` covering `ion/Import.ts` parity (`new`, `getId`, `getText`, `add`) against system-symbol-table offset semantics from upstream `Import.spec.ts`.
...
- `json-pack` now has an upstream-mapped integration matrix at `crates/json-joy-json-pack/tests/upstream_port_codecs_matrix.rs` covering `codecs` wrappers from upstream `codecs/*` (`Codecs`, `cbor`, `msgpack`, `json`, `types`), including id/format identity, trait-based encode/decode behavior, and aggregate `Codecs` holder roundtrip parity.
...
- `crates/json-joy-json-pack/src/nfs/v3/**`: upstream NFSv3 codec family is intentionally out of scope for now; this port currently prioritizes core CRDT and general-purpose codec parity.
