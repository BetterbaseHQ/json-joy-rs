...
fn upstream_port_patch_clock_codec_matrix_matches_fixtures() {
    ...
    for entry in manifest["fixtures"].as_array().expect("fixtures array") {
        ...
        let fixture: Value = serde_json::from_str(
            &fs::read_to_string(dir.join(entry["file"].as_str().expect("fixture file"))).expect("fixture"),
        )
        .expect("fixture json");

        let expected_table_bytes =
            from_hex(fixture["expected"]["clock_table_binary_hex"].as_str().expect("clock table hex"));
        ...
        let expected_table: Vec<LogicalClockBase> = fixture["expected"]["clock_table"]
            ...
            .collect();
        assert_eq!(table, expected_table, "table mismatch for {}", fixture["name"]);

        let encoded = encode_clock_table(&table);
        assert_eq!(encoded, expected_table_bytes, "clock table encode mismatch for {}", fixture["name"]);

        for rel in fixture["expected"]["relative_ids"].as_array().expect("relative ids") {
            ...
            let decoded_id = decode_with_clock_table(&table, session_index, time_diff).expect("decode id");
            let exp_sid = rel["decoded_id"][0].as_u64().expect("decoded sid");
            let exp_time = rel["decoded_id"][1].as_u64().expect("decoded time");
            assert_eq!(decoded_id.sid, exp_sid, "decoded sid mismatch for {}", fixture["name"]);
            assert_eq!(decoded_id.time, exp_time, "decoded time mismatch for {}", fixture["name"]);
        }
    }
}
