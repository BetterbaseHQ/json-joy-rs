//! JSONPath parser implementation (RFC 9535).
//!
//! Rust divergence note: this module holds the implementation behind the
//! upstream-facing `JsonPathParser` facade because `parser.rs` is used to mirror
//! upstream `Parser.ts`.
...
impl<'a> ParserImpl<'a> {
    ...
    /// Parse a JSONPath expression and include parser position on failure.
    pub fn parse_with_position(input: &'a str) -> Result<JSONPath, (ParseError, usize)> {
        let mut parser = Self { input, pos: 0 };
        parser.parse_path().map_err(|err| (err, parser.pos))
    }
    ...
    fn parse_bracket_selector(&mut self) -> Result<Selector, ParseError> {
        match self.peek() {
            ...
            Some('?') => {
                self.advance();
                self.skip_whitespace();
                let has_paren = if self.peek() == Some('(') {
                    self.advance();
                    true
                } else {
                    false
