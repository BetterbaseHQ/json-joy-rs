...
fn con_schema(value: serde_json::Value) -> Schema {
    Schema::Con(ConSchema { base: SchemaBase::default(), value })
}

fn ref_schema(name: &str) -> Schema {
    Schema::Ref(RefSchema { base: SchemaBase::default(), ref_: name.to_string() })
}
...
fn key(k: &str, v: Schema) -> KeySchema {
    KeySchema { base: SchemaBase::default(), key: k.to_string(), value: Box::new(v), optional: None }
}

fn key_opt(k: &str, v: Schema) -> KeySchema {
    KeySchema { base: SchemaBase::default(), key: k.to_string(), value: Box::new(v), optional: Some(true) }
}

fn obj(keys: Vec<KeySchema>) -> Schema {
    Schema::Obj(ObjSchema { keys, ..Default::default() })
}
...
fn schema_base() -> AliasSchema {
    alias("SchemaBase", obj(vec![
        ...
        key_opt("deprecated", obj(vec![
            key_opt("info", str_schema()),
        ])),
...
fn num_schema_def() -> AliasSchema {
    alias("NumSchema", obj(vec![
        key_opt("title", str_schema()),
        key_opt("intro", str_schema()),
        key_opt("description", str_schema()),
        key("kind", con_schema(json!("num"))),
        key_opt("format", or_schema(vec![
            con_schema(json!("i")), con_schema(json!("u")), con_schema(json!("f")),
            con_schema(json!("i8")), con_schema(json!("i16")), con_schema(json!("i32")),
            con_schema(json!("i64")), con_schema(json!("u8")), con_schema(json!("u16")),
            con_schema(json!("u32")), con_schema(json!("u64")),
            con_schema(json!("f32")), con_schema(json!("f64")),
        ])),
...
fn str_schema_def() -> AliasSchema {
    alias("StrSchema", obj(vec![
        key_opt("title", str_schema()),
        key_opt("intro", str_schema()),
        key_opt("description", str_schema()),
        key("kind", con_schema(json!("str"))),
        key_opt("format", or_schema(vec![
            con_schema(json!("ascii")),
            con_schema(json!("utf8")),
        ])),
...
fn bin_schema_def() -> AliasSchema {
    alias("BinSchema", obj(vec![
        ...
        key_opt("format", or_schema(vec![
            con_schema(json!("json")), con_schema(json!("cbor")),
            con_schema(json!("msgpack")), con_schema(json!("resp3")),
            con_schema(json!("ion")), con_schema(json!("bson")),
            con_schema(json!("ubjson")), con_schema(json!("bencode")),
        ])),
        key_opt("min", num_schema()),
        key_opt("max", num_schema()),
    ]))
...
fn json_schema_def() -> AliasSchema {
    alias("JsonSchema", or_schema(vec![
