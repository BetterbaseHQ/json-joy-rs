...
pub fn create_from_base64_bin(
    ...
    let pad_byte = if do_expect_padding { pad.as_bytes()[0] } else { 0 };

    Ok(move |view: &[u8], offset: usize, length: usize| -> Result<Vec<u8>, Base64Error> {
        if length == 0 {
            return Ok(Vec::new());
        }
        ...
        while i < main_end {
            ...
            let sextet0 = if (octet0 as usize) < table.len() { table[octet0 as usize] } else { -1 };
            let sextet1 = if (octet1 as usize) < table.len() { table[octet1 as usize] } else { -1 };
            let sextet2 = if (octet2 as usize) < table.len() { table[octet2 as usize] } else { -1 };
            let sextet3 = if (octet3 as usize) < table.len() { table[octet3 as usize] } else { -1 };
