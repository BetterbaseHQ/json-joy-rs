...
use super::{
    constants::{tuple_index, HEADER_STACKING_SHIFT, HEADER_X1_ANCHOR_BIT, HEADER_X2_ANCHOR_BIT},
...
impl Slices {
    ...
    pub fn ins(
        ...
        let header_bits =
            ((stacking as u64) << HEADER_STACKING_SHIFT) | x2_anchor_bit | x1_anchor_bit;
            ...
            'outer: for chunk in arr.rga.iter() {
                ...
                if let Some(data) = &chunk.data {
                    for (offset, &data_id) in data.iter().enumerate() {
                        if data_id == vec_id {
                            found = Some(Tss::new(chunk.id.sid, chunk.id.time + offset as u64, 1));
                            break 'outer;
                        }
                    }
                }
    ...
    pub fn get(&self, model: &Model, vec_id: Ts) -> Option<Slice> {
        ...
        if present {
            deserialize_slice(model, vec_id)
        } else {
            None
        }
    }
...
fn pack_to_ts(pv: &PackValue) -> Option<Ts> {
    let PackValue::Array(arr) = pv else {
        return None;
    };
    if arr.len() < 2 {
        return None;
...
fn deserialize_slice(model: &Model, vec_id: Ts) -> Option<Slice> {
    ...
    let x1_anchor = if header & HEADER_X1_ANCHOR_BIT != 0 {
        Anchor::After
    } else {
        Anchor::Before
    };
    let x2_anchor = if header & HEADER_X2_ANCHOR_BIT != 0 {
        Anchor::After
    } else {
        Anchor::Before
    };
