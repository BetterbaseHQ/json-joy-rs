...
| less-db compatibility lifecycle apply | `crates/json-joy-core/src/less_db_compat.rs` | partial | yes | yes | partial | no | `apply_patch` is native; `create_model` and unsupported `diff_model` shapes currently use oracle fallback. Baseline inventory: 10 less-db diff steps currently native-covered (`tests/lessdb_model_manager_from_fixtures.rs`). |
