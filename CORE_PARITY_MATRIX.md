...
| `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/fanout/events) | no direct Rust equivalent yet (only `less_db_compat` orchestration) | missing | no | partial | no | yes | Upstream API surface is broad and currently not represented by a first-class Rust runtime API layer. |
| `json-crdt/model/Model.ts` higher-level lifecycle (`fromPatches`, `applyBatch`, schema-aware `load`) | partial across `model.rs`, `model_runtime.rs`, `less_db_compat.rs` | partial | partial | partial | partial | yes | Binary load/fork/apply paths exist, but full upstream lifecycle surface is not yet mapped 1:1. |
...
| `json-crdt-diff/JsonCrdtDiff.diffDstKeys` | no Rust equivalent | missing | no | no | no | yes | Upstream destination-key-only diff mode is not yet exposed/covered in Rust. |
...
| `json-crdt-patch/compaction.ts` | no Rust equivalent | missing | no | no | no | yes | Patch compaction semantics are currently unimplemented in Rust core. |
| `json-crdt-patch/codec/{compact,compact-binary,verbose}` | no Rust equivalent | missing | no | no | no | yes | Binary canonical codec is implemented; alternate patch codecs are not yet ported. |
