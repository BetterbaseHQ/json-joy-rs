...
| `json-crdt-patch/codec/{compact,compact-binary,verbose}` | `crates/json-joy-core/src/patch_compact_codec.rs`, `crates/json-joy-core/src/patch_verbose_codec.rs` | partial | partial | no | no | yes | Native `compact` + `verbose` codec encode/decode baselines added; `compact-binary` remains unported. |
