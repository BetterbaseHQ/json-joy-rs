...
| `json-crdt-diff/JsonCrdtDiff.ts` | `crates/json-joy-core/src/diff_runtime/mod.rs` | partial | yes | yes | partial | yes | Runtime diff path is native-only and now enforces root-replace catch-all for runtime-core JSON families; fixture floor expanded to `model_diff_parity >= 300` and native-support inventory keeps `UnsupportedShape` at zero for covered runtime-core flows. Matrix test coverage: `tests/upstream_port_diff_matrix.rs`, `tests/upstream_port_diff_native_support_matrix.rs`. |
| `json-crdt-diff/JsonCrdtDiff.diffDstKeys` | `crates/json-joy-core/src/diff_runtime/mod.rs` (`diff_model_dst_keys_to_patch_bytes`) | partial | partial | yes | partial | yes | Destination-key-only diff mode now falls back through native diff rather than surfacing `UnsupportedShape` for runtime-core JSON pathways; fixture floor expanded to `model_diff_dst_keys >= 80`. |
...
| less-db compatibility lifecycle apply | `crates/json-joy-core/src/less_db_compat.rs` | partial | yes | yes | partial | yes | `create_model`, `diff_model`, and `apply_patch` are native in production; compatibility diff no longer branches on `UnsupportedShape` for runtime-core JSON families. less-db fixture floor expanded to `>=90` (`tests/lessdb_model_manager_from_fixtures.rs`). |
