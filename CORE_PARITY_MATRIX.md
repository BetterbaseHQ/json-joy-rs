...
| `util/diff/{str,bin,line}` algorithm-level parity | `crates/json-joy-core/src/util_diff/{str,bin}.rs` (+ pending `line`) | partial | partial | no | no | yes | Native baseline now includes direct `str` + `bin` helper modules with upstream-mapped matrix tests; `line` parity remains pending. |
