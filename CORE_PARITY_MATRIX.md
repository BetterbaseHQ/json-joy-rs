...
| `json-crdt-patch/codec/{compact,compact-binary,verbose}` | `crates/json-joy-core/src/patch_compact_codec.rs` (+ pending modules) | partial | partial | no | no | yes | Native `compact` codec encode/decode baseline added; `compact-binary` and `verbose` remain unported. |
