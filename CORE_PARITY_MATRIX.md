...
| `json-crdt/codec/structural/binary/*` encode | `crates/json-joy-core/src/model.rs` (+ new encoder module) | partial | partial | yes | partial | yes | Logical structural encoding now follows upstream `ClockEncoder` traversal semantics; apply-replay binary parity inventory is 30/30 (`tests/upstream_port_model_encode_matrix.rs`). |
